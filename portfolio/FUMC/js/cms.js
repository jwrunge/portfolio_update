...
function bind_buttons()
{
	...
	$('a.undo_delete').on('click', function()
	{
		var content_block = $(this).closest('.editing_block');
			...
			{					
				...
				else
				{
					if($(this).find('.add_image').attr('src') == 'media/core/addImage.png')
						...
						data['image'].push($(this).find('.add_image').attr('src'));
						
					data['name'].push($(this).find('.staff_name').val());
					data['title'].push($(this).find('.staff_title').val());
					data['email'].push($(this).find('.staff_email').val());
				}
			});
