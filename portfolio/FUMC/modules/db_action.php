<?php
	...
	if($_POST['page'] == 'staff.php')
	{
		//Delete everything
		alter_data($con, "DELETE FROM staff");
		
		//Insert new information
		if($_POST['data'])
			$data = insert_data($con, "INSERT INTO staff (!) VALUES (#)", $_POST['data']);
			...
			if(in_array($image_type, array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG, IMAGETYPE_BMP)))
			{
				...
				echo "<div class='thumbnail' style='cursor: default'>";
					echo "<div class='controls'>" .
						"<img class='renamer' data-filename='$filename' src='media/core/edit.png' alt='rename image'/>" .
						"<img class='mover' data-filename='$filename' src='media/core/move.png' alt='move image'/>" .
						"<img class='deleter' data-filename='$filename' src='media/core/x.png' alt='delete image'/>" . 
					"</div>";
					
					//substr because the ../ is evaluated by the file running ajax... which is in a different directory :-)
					echo "<img src='" . substr($image, 3) . "' alt=''/>";
	...
	{
		//Delete everything
		alter_data($con, "DELETE FROM headers WHERE page=?", $_POST['page']);
		alter_data($con, "DELETE FROM content WHERE page=?", $_POST['page']);
		
		//Insert new information
		insert_data($con, "INSERT INTO headers (page, value, image_path, orientation) VALUES (?, ?, ?, ?)", null, [$_POST['page'], $_POST['data']['header_value'], $_POST['data']['header_image'], $_POST['data']['header_orientation']]);
