...
*/
function switchin(switch_in, switch_out, options) //callback = null, track_order = null)
{		
...
window.addEventListener('popstate', function(e) {
	...
	{
		if(e.state)
			set_state(e.state, false);
	}
});

//State = array of screens to switch to; log_state = bool telling if we should push new state or not
function set_state(state, log_state)
{
    if(log_state == undefined) log_state = true;
    if(log_state) 
    {
        //Push new state
        var new_url = '';
        ...
        {
            if(state[i]) new_url = state[i];
        }
        history.pushState(state, null, new_url);
    }
    
     $('html, body').velocity('scroll', {duration: 500, easing: 'ease-in-out', complete: function(){
